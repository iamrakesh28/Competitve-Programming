#include <bits/stdc++.h>

using namespace std;

struct tree{
	long int min;
	int *seq;
	int size;
};

long int minimum(long int a,long int  b,int *num)
{
	if(a < b)
	{
		(*num) = 1;
		return a;
	}
	(*num) = 2;
	return b;
}

void merge(int *small,int *big,int size)
{
	for(int i=0;i<size;++i)
	{
		big[i]=small[i];
	}
	
}
void infinity(long int *arr,int n,int pos,int count,long int sum)
{
	int num;
	tree *node = new tree[count];
	for(int i=count-1;i>=0;--i)
	{
		int val = 2*i+pos;
		if(i+2 < count)
		{
			if(i+3 < count)
			{
				node[i].min=minimum(node[i+2].min,node[i+3].min,&num)-2*arr[val];
				if(num==1)
				{
					 node[i].size= node[i+2].size+1;
					 node[i].seq = new int [node[i].size];
					 merge(node[i+2].seq,node[i].seq,node[i+2].size);
				
				}
				else
				{
					node[i].size= node[i+3].size+1;
					node[i].seq = new int [node[i].size];
					merge(node[i+3].seq,node[i].seq,node[i+3].size);
				}
				node[i].seq[(node[i].size)-1] = val;
			}
			else
			{
				node[i].size=2;
				node[i].min=node[i+2].min-2*arr[val];
				node[i].seq = new int [2];
				merge(node[i+2].seq,node[i].seq,1);
				node[i].seq[1] = val;
				//node[i].seq[1].push_back(val);
			}
		}
		else
		{
			node[i].size=1;
			node[i].min=sum-2*arr[val];
			node[i].seq = new int[1];
			node[i].seq[0]=val;
		}
	}
	int j=0;
	if(node[0].min > node[1].min)
	{
		j=1;
	}
	for(int i=0;i<node[j].size;++i)
	{
		int val=node[j].seq[i];
		arr[val]=(-1)*arr[val];
	}
}

void peak(long int *arr,bool *mark,int n)
{
	long int val;
	if(arr[0]>=arr[1])
	mark[0]=false;
	else
	mark[0]=true;
	for(int i=1;i<n-1;++i)
	{
		if((arr[i]<arr[i-1]) && (arr[i]<arr[i+1]))
		mark[i]=true;
		else
		mark[i]=false;			
	}
	if(arr[n-1]<arr[n-2])
	mark[n-1]=true;
	else
	mark[n-1]=false;
	for(int i=0;i<n;++i)
	{
		if(mark[i]==true)
		{
			if((i<n-2) && (mark[i+2]==true))
			{
				val = arr[i+1]-arr[i]-arr[i+2];
				if(val<=0)
				continue;
			}
			if(i>=2 && mark[i-2]==true)
			{
				val=arr[i-1]-arr[i]-arr[i-2];
				if(val <= 0)
				continue;
			}
			mark[i]=false;
			arr[i] -= 2*arr[i];
		}	
	}
}

int main()
{
	int q;
	scanf("%d",&q);
	while(q--)
	{
		int n,count=0,pos;
		scanf("%d",&n);
		long int sum=0,*arr = new long int[n];
		bool *mark = new bool[n];	
		for(int i=0;i<n;++i)
		{
			scanf("%ld",&arr[i]);
			sum += arr[i];
		}
		peak(arr,mark,n);
		int j;	
		count=0;
		for(j=0;j<n;++j)
		{
			if(mark[j]==true)
			{
				count++;
				j++;
			}
			else if((j>=2) && mark[j-2]==true)
			{
				pos=j-2*count;
				infinity(arr,n,pos,count,sum);
				count=0;
			}
		}
		if((j>=2) && mark[j-2]==true)
		{
			pos=j-2*(count);
			infinity(arr,n,pos,count,sum);
			count=0;
		}
		for(int i=0;i<n;++i)
		{
			printf("%ld ",arr[i]);
		}
		printf("\n");
	}
	
}
