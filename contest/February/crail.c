...
int x[N], y[N], int prime[N], psize = 0;
vector <int> newfact[N];
vector <int> fact[N], power[N];
...
void factor(int n) {
  for (int i = 2; i <= n; ++i) {
    ...
    for (int p = 0; p < psize; ++p) {
      if ((long long)prime[p] * prime[p] > num)
	break;
      if (num % p == 0) {
	fact[i].push_back(p);
	power[i].push_back(1);
...
void backtrack(int num, int i, int n, long long val) {
  ...
  long long times = 1;
  for (int j = 0; j <= power[num][i]; ++j) {
    if (val * times > N)
      return;
    assert(fact[num][i] > 0);
    backtrack(num, i + 1, n, val * times);
    times = times * fact[num][i];
  }
  return;
}
